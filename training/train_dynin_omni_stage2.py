...
def _resolve_stage2_model_sources(
    config,
...
def _get_eval_cfg(config) -> Dict[str, Any]:
    eval_cfg = _to_plain_container(getattr(config, "evaluation", {})) or {}
    if not isinstance(eval_cfg, dict):
        return {}
    return eval_cfg


def _get_dataset_params_cfg(config) -> Dict[str, Any]:
    dataset = getattr(config, "dataset", None)
    params = _to_plain_container(getattr(dataset, "params", {})) or {}
...
def _resolve_optional_path(
    path_value: Optional[Union[str, os.PathLike]],
) -> Optional[Path]:
    if not path_value:
        return None
...
def _parse_eval_steps(raw_steps: Any) -> set[int]:
    cfg_value = _to_plain_container(raw_steps)
    if cfg_value is None:
        return set()
    if isinstance(cfg_value, (int, float, str)):
        cfg_value = [cfg_value]
    if not isinstance(cfg_value, (list, tuple, set)):
        return set()
    parsed_steps: set[int] = set()
    for step in cfg_value:
        try:
            parsed = int(step)
        except (TypeError, ValueError):
            continue
        if parsed > 0:
            parsed_steps.add(parsed)
    return parsed_steps
...
def collate_fn_x2i(batch):
    ...
    if has_i2i_sample:
        # If any I2I samples exist, use this batch only for I2I.
        t2i_texts = []
        t2i_images = []
...
def generate(
    ...
    if attention_mask is not None and 0.0 in attention_mask:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("attention_bias shape: %s", tuple(attention_bias.shape))
    else:
        attention_bias = None
...
def evaluate_t2i(
    ...
    dataset_cfg = _get_dataset_params_cfg(config)
    prompts_file = eval_cfg.get(
        "t2i_prompts_file", dataset_cfg.get("validation_prompts_file")
    )
    prompts_path = _resolve_optional_path(prompts_file)
    ...
    if not prompts_path.exists():
        logger.warning(
            f"T2I prompts file '{prompts_file}' not found. Skipping T2I evaluation."
        )
        return
    try:
...
def evaluate_s2s(
    ...
    logger.info("***** Running S2S Evaluation *****")
    unwrapped_model = accelerator.unwrap_model(model)
    unwrapped_model.eval()
    ...
    s2s_eval_dir = getattr(dataset_cfg, "s2s_eval_dir", "MMaDA/validation_prompts/s2s")

    base_path = Path(s2s_eval_dir)
    if not base_path.is_absolute():
        base_path = Path.cwd() / base_path
        if not base_path.exists():
            repo_root = Path(__file__).resolve().parents[2]
            alt_path = repo_root / s2s_eval_dir
            if alt_path.exists():
                base_path = alt_path

    if not base_path.exists():
        logger.warning(
            f"S2S evaluation directory '{s2s_eval_dir}' not found. Skipping S2S evaluation."
        )
        return

    audio_exts = {".wav", ".flac", ".mp3", ".ogg", ".m4a"}
    wav_files = sorted(
        p for p in base_path.iterdir() if p.is_file() and p.suffix.lower() in audio_exts
    )
    if not wav_files:
        logger.warning(
            f"No audio files found in '{base_path}'. Skipping S2S evaluation."
        )
        return
    ...
    mask_token_id = unwrapped_model.config.mask_token_id
    codebook_size = int(getattr(config.model.dynin_omni, "codebook_size", 8192))

    speech_vocab_start = len(uni_prompting.text_tokenizer) + codebook_size
    audio_codebook_size = 4096
    if audio_codebook_size <= 0:
        logger.warning(
            "Computed audio codebook size is non-positive. Skipping S2S evaluation."
        )
        return

    a_tokens = uni_prompting.text_tokenizer(
        "<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n", return_tensors="pt"
    ).input_ids
    soa_len = uni_prompting.sptids_dict["<|soa|>"].numel()
    eoa_len = uni_prompting.sptids_dict["<|eoa|>"].numel()
    asst_header_len = a_tokens.shape[1]
    max_audio_len = getattr(uni_prompting, "max_audio_len", 256)
    max_generatable = 384

    offset = len(uni_prompting.text_tokenizer) + codebook_size
    device = accelerator.device

    output_root = (
        Path(config.experiment.output_dir) / "eval_s2s" / f"step_{global_step}"
    )
    output_root.mkdir(parents=True, exist_ok=True)

    table = wandb.Table(
        columns=["Audio ID", "Source Audio", "Generated Audio", "Token Count"]
    )

    for audio_path in wav_files:
        try:
            user_tokens = vq_model_audio.encode(str(audio_path))
        except Exception as exc:
            logger.error(f"Failed to encode '{audio_path}': {exc}")
            continue
        ...
        user_tokens = user_tokens.to(device=device, dtype=torch.long)
        if user_tokens.numel() == 0:
            logger.warning(
                f"Encoded audio from '{audio_path}' produced no tokens. Skipping sample."
            )
            continue

        # Use a fixed assistant placeholder length so generation is not limited by user input duration.
        assistant_len = max_generatable
        if assistant_len <= 0:
            logger.warning(
                f"Assistant placeholder length for '{audio_path}' is non-positive. Skipping sample."
            )
            continue
        ...
        input_ids, attention_mask = uni_prompting(
            ([user_shifted], [assistant_placeholder]), "s2s_gen"
        )

        try:
            generated_sequences = unwrapped_model.t2s_generate_mmu_like(
                input_ids=input_ids,
                max_new_tokens=384,
                steps=384,
                block_length=384,
                temperature=config.training.get("s2s_generation_temperature", 1.0),
                cfg_scale=config.training.get("s2s_guidance_scale", 3.5),
            ...
            logger.error(f"Generation failed for '{audio_path}': {exc}")
            continue

        if not generated_sequences:
            logger.warning(f"No tokens generated for '{audio_path}'. Skipping sample.")
            continue

        gen_tokens = generated_sequences[0]
        if isinstance(gen_tokens, torch.Tensor):
            gen_tokens = gen_tokens.detach().cpu()
        token_list = gen_tokens.tolist()
        if not token_list:
            logger.warning(
                f"Generated token list empty for '{audio_path}'. Skipping sample."
            )
            continue

        speech_unit_str = "".join([f"<|speech_{int(token)}|>" for token in token_list])
        output_path = output_root / f"{audio_path.stem}_reply.wav"

        try:
            vq_model_audio.decode(
                speech_unit_str, condition=condition, output_wav_file=str(output_path)
            )
        except Exception as exc:
            logger.error(f"Decoding failed for '{audio_path}': {exc}")
            continue

        table.add_data(
            audio_path.name,
            wandb.Audio(str(audio_path), caption="source"),
            wandb.Audio(str(output_path), caption="generated"),
            len(token_list),
        )
    ...
    if row_count > 0:
        accelerator.log({"eval/s2s_samples": table}, step=global_step)
    else:
        logger.warning("S2S evaluation produced no loggable samples.")
...
def evaluate_text(model, uni_prompting, config, accelerator, global_step):
    ...
    prompts_file = getattr(
        dataset_cfg, "text_eval_prompts_file", "MMaDA/validation_prompts/math.txt"
    )
...
def evaluate_s2t(
    ...
    unwrapped_model.eval()
    eval_cfg = _get_eval_cfg(config)
    s2t_root = _resolve_optional_path(eval_cfg.get("s2t_librispeech_root"))
    if s2t_root is None or not s2t_root.exists():
        logger.warning(
            "evaluation.s2t_librispeech_root is not set or not found. Skipping S2T evaluation."
        )
        return
    ...
    try:
        ...
        s2t_eval_dataset = S2TEvalDataset(s2t_eval_dataset_raw, root_path=str(s2t_root))
...
def evaluate_t2s(
    ...
    unwrapped_model.eval()
    eval_cfg = _get_eval_cfg(config)
    audio_output_dirname = str(eval_cfg.get("audio_output_dirname", "eval_audio"))

    # Load dataset and Whisper model.
    try:
        ...
        os.makedirs(
            os.path.join(config.experiment.output_dir, audio_output_dirname),
            exist_ok=True,
        )
    except Exception as e:
    ...
    output_dir_per_step = os.path.join(
        config.experiment.output_dir, audio_output_dirname, f"step_{global_step}"
    )
    ...
    for batch in tqdm(t2s_dataloader, desc="T2S Evaluation"):
        ...
        # Decode output and run Whisper.
        for i in range(batch_size):
            gt = gt_texts[i].rsplit("\n", 1)[-1].strip()

            gen_speech_tokens = output_ids[i]

            # Remove padding and eos tokens if needed.
...
def evaluate_t2s_mmu_like(
    ...
    output_dir_per_step = os.path.join(
        config.experiment.output_dir, audio_output_dirname, f"step_{global_step}_mmu"
    )
    os.makedirs(output_dir_per_step, exist_ok=True)
...
def evaluate_v2t(
    ...
    eval_cfg = _get_eval_cfg(config)
    dataset_cfg = _get_dataset_params_cfg(config)
    video_root = eval_cfg.get("v2t_video_root", dataset_cfg.get("video_root"))
    video_root_path = _resolve_optional_path(video_root)
    if video_root_path is None or not video_root_path.exists():
        logger.warning(
    ...
    for file_name in tqdm(
        ...
        video_path = os.path.join(str(video_root_path), file_name)
        ...
        logger.info("V2T eval caption (%s): %s", file_name, text)
...
def evaluate_v2s(
    ...
    logger.info("***** Running V2S Qualitative Evaluation *****")
    unwrapped_model = accelerator.unwrap_model(model)
    unwrapped_model.eval()
    eval_cfg = _get_eval_cfg(config)
    dataset_cfg = _get_dataset_params_cfg(config)
    ...
    try:
        ...
        logger.error(f"Failed to load Whisper model for V2S eval: {exc}")
        return

    video_root = eval_cfg.get(
        "v2s_video_root",
        eval_cfg.get("v2t_video_root", dataset_cfg.get("video_root")),
    )
    video_root_path = _resolve_optional_path(video_root)
    if video_root_path is None or not video_root_path.exists():
        logger.warning(
            f"V2S eval root '{video_root}' not found. Skipping V2S evaluation."
        )
        return
    ...
    if not file_list:
        logger.warning(
            f"No .mp4 files found in '{video_root_path}'. Skipping V2S evaluation."
        )
        return

    prompt_candidates = V2S_INSTRUCTION
    results_table = wandb.Table(
        columns=["Video ID", "Question", "Whisper Transcript", "Generated Audio"]
    )

    device = unwrapped_model.device
    mask_token_id = unwrapped_model.config.mask_token_id
    eoa_token_id = int(uni_prompting.sptids_dict["<|eoa|>"][0].item())
    audio_codebook_size = 4096
    max_audio_tokens = int(
        getattr(
            uni_prompting,
            "max_audio_len_short",
            config.dataset.preprocessing.max_aud_length_short,
        )
    )
    max_new_tokens = max(1, max_audio_tokens - 1)
    block_length = 128 if max_new_tokens % 128 == 0 else max_new_tokens

    v2s_audio_output_dirname = str(
        eval_cfg.get("v2s_audio_output_dirname", "eval_audio_v2s")
    )
    output_dir = os.path.join(
        config.experiment.output_dir, v2s_audio_output_dirname, f"step_{global_step}"
    )
    os.makedirs(output_dir, exist_ok=True)

    for file_name in tqdm(
        file_list[:], desc="V2S Evaluation", disable=not accelerator.is_main_process
    ):
        video_path = os.path.join(str(video_root_path), file_name)
        ...
        if len(frames) < 8:
            logger.warning(f"Skipping {file_name}: insufficient frames.")
            continue
        ...
        try:
            ...
            logger.error(f"Failed to encode video {file_name}: {exc}")
            continue
        video_tokens = video_tokens.view(1, -1)

        question = random.choice(prompt_candidates)
        prompt_text = f"<|start_header_id|>user<|end_header_id|>\n{question}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n"
        prompt_tensor = uni_prompting.text_tokenizer(
            prompt_text, return_tensors="pt"
        ).input_ids.to(device)
        ...
        try:
            seq_ids, attn_mask = uni_prompting(
                (video_tokens, [prompt_text], [audio_placeholder]), "v2s_gen"
            )
        except Exception as exc:
            logger.error(f"Prompt construction failed for {file_name}: {exc}")
            continue

        input_ids = seq_ids.to(device)
        attention_mask = attn_mask.to(device)

        try:
            generated_list = unwrapped_model.t2s_generate_mmu_like(
                input_ids=input_ids,
                max_new_tokens=256,
                steps=256,
                block_length=256,
            ...
            logger.error(f"Generation failed for {file_name}: {exc}")
            continue

        generated_tokens = generated_list[0]
        if isinstance(generated_tokens, torch.Tensor):
            generated_tokens = generated_tokens.detach().cpu()

        collected = []
        for token in generated_tokens.tolist():
            if token == eoa_token_id or token >= audio_codebook_size:
                break
            if token >= 0:
                collected.append(token)

        if not collected:
            logger.warning(f"No valid audio tokens generated for {file_name}.")
            continue

        speech_unit_for_decode = "".join(f"<|speech_{tok}|>" for tok in collected)
        output_wav_path = os.path.join(output_dir, f"{Path(file_name).stem}_v2s.wav")
        try:
            vq_model_audio.decode(
                speech_unit_for_decode,
                condition="gender-female_emotion-neutral_speed-normal_pitch-normal",
            ...
            logger.error(f"Decoding failed for {file_name}: {exc}")
            continue
        ...
        results_table.add_data(
            ...
            question,
            whisper_text,
            wandb.Audio(output_wav_path, caption=whisper_text),
        )

    if len(results_table.data) == 0:
        logger.warning("V2S evaluation produced no samples to log.")
        return

    accelerator.log({"eval/v2s_qualitative_samples": results_table}, step=global_step)
...
def run_evaluation(
    ...
    if accelerator.is_main_process:
        evaluate_text(model, uni_prompting, config, accelerator, global_step)
        evaluate_t2i(
            model, vq_model_image, uni_prompting, config, accelerator, global_step
        )
        evaluate_i2i(
        ...
        # V2S and S2S evaluations are intentionally disabled for stage2.
...
def main():
    ...
    # Log configuration from every process for debugging.
    logging.basicConfig(
    ...
    # Trackers initialize automatically on the main process.
    if accelerator.is_main_process:
    ...
    # Set the training seed when provided.
    ...
    tokenizer_source, model_source, vq_image_source, vq_audio_source = (
        _resolve_stage2_model_sources(config, model_cfg=dynin_omni_cfg)
    )
    logger.info(
    ...
    if accelerator.is_main_process:
        token_keys = sorted(uni_prompting.sptids_dict.keys())
        logger.info("Special token keys: %s", token_keys)
    ...
    # Load the VQ model for image tokenization.
    ...
    # Configure speech-token caching.
    ...
    s2s_precomputed_token_sources: list[dict[str, Any]] = []
    ...
    def _load_external_speech_tokens(
        audio_path: Union[str, Path],
    ) -> Optional[torch.Tensor]:
        if not isinstance(audio_path, (str, os.PathLike)):
            return None
        if not s2s_precomputed_token_sources:
            return None
        abs_key = os.path.abspath(str(audio_path))
        digest = hashlib.sha1(abs_key.encode("utf-8")).hexdigest()
        shard_a, shard_b = digest[:2], digest[2:4]
        filename = f"{digest}.pt"
        for source in s2s_precomputed_token_sources:
            token_path = source["root"] / shard_a / shard_b / filename
            ...
            try:
                ...
                logger.warning(
                    "Failed to load speech2speech cached tokens from %s (%s); skipping.",
                    token_path,
                    exc,
                )
                continue
            ...
            logger.warning(
                "Unexpected type (%s) for speech2speech cached tokens at %s; ignoring.",
                type(tokens),
                token_path,
            )
        return None
    ...
    def _load_cached_audio_tokens(
        ...
        external_tokens = _load_external_speech_tokens(audio_path)
        if external_tokens is not None:
            if len(audio_token_cache_mem) < speech_cache_max_items:
                audio_token_cache_mem[cache_key] = external_tokens
            return external_tokens.clone()
    ...
    model_config_path = dynin_omni_cfg.get("model_config_path", None)
    if isinstance(model_config_path, str):
        stripped_path = model_config_path.strip()
        model_config_path = (
            stripped_path if stripped_path and stripped_path.upper() != "NONE" else None
        )
    if model_config_path is not None:
        resolved_model_config = (
            _resolve_optional_path(model_config_path)
            if isinstance(model_config_path, (str, os.PathLike))
            else None
        )
        model_load_kwargs["config"] = (
            str(resolved_model_config)
    ...
    # Do not apply weight decay to bias, layer norm, and embeddings.
    ...
    batch_size_s2s_cfg = 0
    batch_size_v2s_cfg = 0
    ...
    if batch_size_mmu_cfg > 0:
        ...
        if mmu_params is None:
            mmu_kwargs = {}
        elif isinstance(mmu_params, dict):
            mmu_kwargs = mmu_params
        else:
            mmu_kwargs = OmegaConf.to_container(mmu_params, resolve=True)
        ...
        if "jsonl_path" in mmu_kwargs:
            dataset_mmu = CambrianInterleavedDataset(**mmu_kwargs)
        else:
            dataset_mmu = TextImageInterleavedDataset(**mmu_kwargs)

    # Load T2I and I2I datasets.
    ...
    # Load language modeling dataset (HF instruction mixture).
    ...
    if batch_size_v2t_cfg > 0:
        ...
        dataset_v2t = VideoCaptionDataset(
            ...
            llavavid_path=speech_cfg.get("llavavid_path", "lmms-lab/LLaVA-Video-178K"),
            num_frames=8,
            ...
            llavavid_skip_video_patterns=speech_cfg.get("llavavid_skip_video_patterns"),
    ...
    # Load speech dataset only when speech flows are enabled.
    speech_enabled = (
    ...
    # S2S is disabled for stage2.
    dataset_s2s = None
    sampler_s2s = None
    train_dataloader_s2s = None
    ...
    # Use distinct DistributedSamplers for each speech dataloader.
    sampler_s2t = None
    sampler_t2s = None
    ...
    if not iterables:
        ...
        if accelerator.is_main_process:
            logger.warning(
                "CombinedLoader has no non-speech iterables; running without LM/MMU combined flows."
            )
    else:
    ...
    num_update_steps_per_epoch_s2s = _num_steps(dataset_s2s, batch_size_s2s_cfg)
    num_update_steps_per_epoch_v2t = _num_steps(dataset_v2t, batch_size_v2t_cfg)
    num_update_steps_per_epoch_v2s = _num_steps(dataset_v2s, batch_size_v2s_cfg)
    ...
    logger.info(
        f"len of Video Speech: {len(dataset_v2s) if dataset_v2s is not None else 0}"
    )
    logger.info(f"len of S2S: {len(dataset_s2s) if dataset_s2s is not None else 0}")
    ...
    # Resume model state from checkpoint when requested.
    global_step = 0
    first_epoch = 0
    start_step = 0
    ...
    if config.experiment.resume_from_checkpoint:
        ...
        if path is not None:
            ...
            # Load sharded safetensors checkpoints when available.
    ...
    def safe_image_get_code(image_tensor_sample: torch.Tensor, sample_index: int):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    msg = (
                        f"[Rank {accelerator.process_index}] s2s image encode failed "
                        f"for sample index {sample_index}: {exc}"
                    )
                    _log_and_flag_failure(msg, exc)
                    return None, msg
    ...
    def _decode_single_image(single_like):
        if single_like is None:
            return None
        if isinstance(single_like, Image.Image):
            return single_like.convert("RGB")

        data_bytes = None

        if isinstance(single_like, (bytes, bytearray)):
            data_bytes = bytes(single_like)
        elif isinstance(single_like, str):
            try:
                data_bytes = base64.b64decode(single_like)
            except (binascii.Error, ValueError):
                if os.path.isfile(single_like):
                    try:
                        with open(single_like, "rb") as fh:
                            data_bytes = fh.read()
                    except OSError:
                        data_bytes = None
        elif isinstance(single_like, dict):
            binary_payload = single_like.get("bytes")
            if binary_payload is not None:
                data_bytes = binary_payload
            else:
                path_value = single_like.get("path")
    ...
    def maybe_decode_image(image_like):
        if isinstance(image_like, (list, tuple)):
            return [_decode_single_image(item) for item in image_like]
        return _decode_single_image(image_like)
    ...
    def prepare_inputs_and_labels_for_text(
        ...
        # Create MLM masks and labels.
        # Truncate inputs to max_seq_lm_input and pad labels to max_seq_len_out.
        tokenized = tokenizer(
            texts,
            truncation=True,
            max_length=max_seq_lm_input,
        )["input_ids"]
        input_ids_lm, prompt_mask, labels_lm = uni_prompting.lm_prompt(
    ...
    def prepare_inputs_and_labels_for_mmu(
        ...
        # 126336 is used for the [MASK] token.
    ...
    eval_on_start = bool(config.experiment.get("eval_on_start", False))
    forced_eval_steps = _parse_eval_steps(config.experiment.get("eval_force_steps", []))
    ...
    for epoch in tqdm(
        ...
        while True:
            ...
            v2t_batch = None
            selected_v2_branch: Optional[str] = None
            t2i_batch = None
            i2i_batch = None
            selected_x2i_branch: Optional[str] = None

            v2_choices: list[str] = []
            if train_dataloader_v2t is not None:
                v2_choices.append("v2t")

            if v2_choices:
                choice_idx = global_step % len(v2_choices)

                selected_v2_branch = v2_choices[choice_idx]
                if selected_v2_branch == "v2t":
            ...
            if train_dataloader_i2i is not None:
                x2i_choices.append("i2i")

            if x2i_choices:
                choice_idx = global_step % len(x2i_choices)

                selected_x2i_branch = x2i_choices[choice_idx]
                if selected_x2i_branch == "t2i":
            ...
            try:
                ...
                # Fallback when reduce is unavailable.
            ...
            device = accelerator.device
            batch_size_v2s = 0
            input_ids_v2s = torch.empty((0, 1), dtype=torch.long, device=device)
            labels_v2s = torch.empty((0, 1), dtype=torch.long, device=device)
            p_mask_v2s = torch.empty((0, 1), dtype=torch.float32, device=device)
            answer_lengths_v2s = torch.empty((0, 1), dtype=torch.long, device=device)
            ...
            if selected_x2i_branch == "t2i" and t2i_batch is not None:
            ...
            if selected_x2i_branch == "i2i" and i2i_batch is not None:
            ...
            video_tensor_speech = (
                video_tensor_speech_raw
                if isinstance(video_tensor_speech_raw, torch.Tensor)
                else torch.empty((0, 1, 1, 1, 1))
            )
            ...
            active_x2i_branch = selected_x2i_branch or "none"
            logger.info(
                f"x2i_branch: {active_x2i_branch}, batch_size_t2i: {batch_size_t2i}, batch_size_i2i: {batch_size_i2i}, batch_size_lm: {batch_size_lm}, "
                f"batch_size_v2t: {batch_size_v2t}, batch_size_v2s: {batch_size_v2s}, batch_size_t2s: {batch_size_t2s_text}, "
                f"batch_size_s2t: {batch_size_s2t}, batch_size_s2s: {batch_size_s2s}, batch_size_mmu: {batch_size_mmu}"
            )
            ...
            # S2S is disabled for stage2.
            input_ids_s2s = torch.empty((0, 1), dtype=torch.long, device=device)
            labels_s2s = torch.empty((0, 1), dtype=torch.long, device=device)
            p_mask_s2s = torch.empty((0, 1), dtype=torch.float32, device=device)
            answer_lengths_s2s = torch.empty((0, 1), dtype=torch.long, device=device)
            ...
            seq_lengths.extend(
                [
                    ...
                    input_ids_v2s.shape[1],
                    input_ids_s2t.shape[1],
                    input_ids_t2s.shape[1],
                ]
            )
            if input_ids_s2s.shape[0] > 0:
                seq_lengths.append(input_ids_s2s.shape[1])
            ...
            answer_lengths_s2s = pad_answer_lengths(answer_lengths_s2s, max_len)
            ...
            # Handle empty text/image batches.
            if batch_size_lm == 0:
            ...
            with accelerator.accumulate(model):
                ...
                # Gather losses across processes for logging.
                ...
                if not torch.isfinite(loss_t2s):
                    if labels_t2s.numel() > 0:
                        ...
                        if valid_mask.any():
                            ...
                            below_count = (
                                (labels_valid < speech_vocab_start).sum().item()
                            )
                            above_count = (
                                (labels_valid >= speech_vocab_end).sum().item()
                            )
                ...
                i2i_coeff = getattr(
                    config.training, "i2i_coeff", config.training.t2i_coeff
                )
                loss = (
                ...
                avg_masking_rate = accelerator.reduce(
                    local_masking_rate, reduction="mean"
                )

                if batch_size_i2i > 0:
                    local_masking_rate_i2i = mask_prob_i2i.float().mean()
                else:
                    local_masking_rate_i2i = torch.tensor(
                ...
                # Log gradient norm before zeroing.
                if (
            ...
            if accelerator.sync_gradients:
                ...
                if log_every is not None and (global_step + 1) % log_every == 0:
                    ...
                    # Reset batch and data time meters for the next log window.
                    batch_time_m.reset()
                    data_time_m.reset()
                ...
                if eval_every is not None:
                    ...
                    if eval_on_start and current_step == 1:
                        should_run_eval = True
                    if current_step in forced_eval_steps:
                        should_run_eval = True
...
def visualize_predictions(*args, **kwargs):
    # This function is kept for compatibility and is not used in the main loop.
    pass
...
def save_checkpoint(model, config, accelerator, global_step, uni_prompting):
    ...
    # Check whether this save exceeds the checkpoint retention limit.
    if accelerator.is_main_process and checkpoints_total_limit is not None:
        checkpoints = os.listdir(output_dir)
        checkpoints = [d for d in checkpoints if d.startswith("checkpoint")]
        checkpoints = sorted(checkpoints, key=lambda x: int(x.split("-")[1]))

        # Keep at most checkpoints_total_limit - 1 old checkpoints before saving.
    ...
    # Retrieve model state on all processes for DeepSpeed stage 3 compatibility.
